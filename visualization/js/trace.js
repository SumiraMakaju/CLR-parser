function renderTraceTable() {
  ...
  parseSteps.forEach((step, i) => {
    ...
    let short = '';
    if      (aType === 'shift')  { const m = step.action.match(/state (\d+)/); short = `s${m ? m[1] : '?'}`; }
    else if (aType === 'reduce') { short = `r${step.reduceProduction}`; }
    else if (aType === 'accept') { short = 'acc'; }
    else                         { short = 'err'; }

    const cls     = aType === 'shift'  ? 'cell-shift'
                  : aType === 'reduce' ? 'cell-reduce'
                  : aType === 'accept' ? 'cell-accept' : '';
